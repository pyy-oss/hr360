...
const MATRIX: Record<Role, Partial<Record<Resource, Action[]>>> = {
  ...
  // Recruteur : périmètre recrutement uniquement (pas de paie, engagement, offboarding).
  recruteur:   { employees:['read'], campaigns:['read'] },
  ...
  // Dirigeant : lecture stratégique (agrégats), sans PII fine ni écriture.
  dirigeant:   { employees:['read'], leaveRequests:['read'], trainingPlans:['read'], objectives:['read'], evaluations:['read'], campaigns:['read'], auditLogs:['read'] },
};
